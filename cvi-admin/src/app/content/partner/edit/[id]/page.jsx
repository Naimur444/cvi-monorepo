...
const EditPartnerPage = () => {
  ...
  // Fetch partner data from API based on ID
  useEffect(() => {
    const fetchPartner = async () => {
      try {
        const response = await api.get(`/partner/${params.id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.patch(`/partner/${params.id}`, formData);
  ...
  const handleDelete = async () => {
    if (
      ...
      try {
        await api.delete(`/partner/${params.id}`);
