        ...
        <div className="bg-white rounded-2xl mt-6 p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              onClick={async () => {
                if (
                  window.confirm(
                    "Are you sure you want to delete this services header? This action cannot be undone."
                  )
                ) {
                  setIsLoading(true);
                  try {
                    await new Promise((r) => setTimeout(r, 1000));
                    console.log("Deleting services header:", headerId);
                    alert("Services header deleted successfully!");
                    router.push("/services/header");
                  } catch (err) {
                    console.error(err);
                    alert("Error deleting services header. Please try again.");
                  } finally {
                    setIsLoading(false);
                  }
                }
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-center gap-4">
              <button
                type="button"
                onClick={() => router.push("/services/header")}
