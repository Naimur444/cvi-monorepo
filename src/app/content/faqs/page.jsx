...
const FaqsPage = () => {
  ...
  const [faqs, setFaqs] = useState([
    {
      id: 1,
      question: "What services does Cloud Vortex Innovation provide?",
      answer:
        "We provide comprehensive software development, web design, mobile app development, and digital transformation services.",
      status: true,
    },
    {
      id: 2,
      ...
      answer:
        "Project timelines vary based on complexity, but most projects range from 2-6 months for completion.",
      status: false,
    },
    {
      id: 3,
  ...
  // Handle content image status toggle
  const handleContentImageStatusToggle = (imageId, newStatus) => {
  ...
  const handleContentImageDelete = (imageId) => {
    if (
      window.confirm(
        "Are you sure you want to delete this content image? This action cannot be undone."
      )
    ) {
      setContentImages((prev) => prev.filter((image) => image.id !== imageId));
    }
  };

  // Handle FAQ status toggle
  const handleFaqStatusToggle = (faqId, newStatus) => {
    setFaqs((prev) =>
      prev.map((faq) =>
  ...
  const contentImageColumns = [
    ...
    },
    {
      ...
      render: (value, row) => (
        <Switch
          checked={value}
          onCheckedChange={(val) => handleContentImageStatusToggle(row.id, val)}
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex gap-2">
          <Link href={`/content/faqs/content-image/edit/${row.id}`}>
            <button
          ...
          <button
            ...
            onClick={() => handleContentImageDelete(row.id)}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="25"
  ...
  const faqColumns = [
    ...
    },
    {
      ...
      render: (value, row) => (
        <Switch
          checked={value}
          onCheckedChange={(val) => handleFaqStatusToggle(row.id, val)}
          className="data-[state=checked]:bg-[#0E4F53]"
        />
      ),
    },
    {
      ...
      render: (_, row) => (
        <div className="flex gap-2">
          <Link href={`/content/faqs/edit/${row.id}`}>
            <button
              className="cursor-pointer p-1 hover:bg-gray-100 rounded transition-colors"
              title="Edit FAQ"
            >
          ...
          <button
            className="p-1 hover:bg-red-50 rounded transition-colors"
            title="Delete FAQ"
            onClick={() => handleFaqDelete(row.id)}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="25"
  ...
  return (
    <section>
      <div className="flex items-center justify-between">
        <MenuItem parent={"Content"} page={"FAQ's"} href={"/content/faqs"} />
          <div className="flex flex-col gap-2">
            <h3 className="text-[#3D3D3D] font-semibold text-lg">
              Select Category
            </h3>

            <select className="w-64 px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-[#0E4F53] focus:border-[#0E4F53] transition">
        ...
        <div className="bg-white rounded-2xl my-6 p-4">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link href={"/content/faqs/content-image"}>
          ...
          <DataTable columns={contentImageColumns} data={contentImages} />
        </div>

        <div className="bg-white rounded-2xl mt-6 p-4">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-[#181818] font-semibold text-xl">FAQ's</h3>
            <Link href={"/content/faqs/add-faqs"}>
              <button className="flex items-center gap-1 justify-center py-2 px-4 bg-[#0E4F53] text-white rounded-lg cursor-pointer transition-all duration-300 ease-out hover:bg-[#0E4F53]/90 hover:scale-105 hover:shadow-lg hover:shadow-[#0E4F53]/25 active:scale-95 active:bg-[#0E4F53]/80 focus:outline-none focus:ring-2 focus:ring-[#0E4F53] focus:ring-offset-2">
                + Add
              </button>
            </Link>
